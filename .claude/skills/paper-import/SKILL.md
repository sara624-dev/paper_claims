...
allowed-tools: Read, Write, Edit, Glob, Grep, AskUserQuestion, Bash(curl *), Bash(ls *), Bash(uv run *), Bash(git *), Bash(date *), WebFetch
---
...
Read ツールの `pages` パラメータで分割して読む（1回最大20ページ。10ページ超のPDFは pages 指定必須）。
優先順: **Abstract / Introduction（貢献の列挙）→ 実験・結果（数値・条件）→ Conclusion / Limitations（限界の自己申告）**。付録は数値の裏取りが必要な場合のみ。
